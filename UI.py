...
def predict():
    ...
    try:
        ...
        messagebox.showinfo("Prediction Result", f"Loan Status: {result['Loan Status']}")
...
entry_applicant_income = create_entry("Applicant Income", 6)
entry_coapplicant_income = create_entry("Coapplicant Income", 7)
entry_loan_amount = create_entry("Loan Amount", 8)
entry_loan_term = create_entry("Loan Amount Term", 9)
...
predict_button.grid(row=10, columnspan=2, pady=20)
